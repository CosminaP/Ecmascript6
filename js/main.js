...
const getIds =  new Promise( (resolve, reject) => {
    //executer function:
    let msg = () => { return "Incomplete request, please try again later."; }
    setTimeout( () => {
    //setting resolved to be executed when the function is succesful
        resolve([123, 15, 25, 24]);
    }, 1500); 
    //setting what to be executed if the function is rejected: 
    // reject(msg())
    // }, 1500); 
})
...
const getRecipe =  recipeId => {
    return new Promise( (resolve, reject) => {
            setTimeout ( () => {
                const recipe =  { title: 'Naan Fluffy Bread', publisher: 'Cosmina Nutrition'}
...
.then( IDs => {
    console.log(IDs);
    //returning a promise
    // the beauty of Promises is the chaining method 
    // meaning we use then to return a promise and then we chain it;
    return getRecipe(IDs[2]);
})
// now we using then method on the promise returned above
.then( recipe => 
    // we are returning what to happen when the promise is resolved succesful:
    console.log(recipe) 
)
.catch( error => {
    console.log(error);
